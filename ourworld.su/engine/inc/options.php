	...
	while ( $row = $db->get_array() ) {
	  ...
	  <div class="col-md-6 media-list media-list-linked">
		<a class="media-link" href="?mod={$row['name']}">
			<div class="media-left"><img src="engine/skins/images/{$row['icon']}" class="img-lg section_icon"></div>
			<div class="media-body">
...
elseif( $action == "syscon" ) {
	...
	if( ! $handle = opendir( "./language" ) ) {
		die( "Невозможно открыть директорию /language/" );
	}
	while ( false !== ($file = readdir( $handle )) ) {
...
HTML;
	...
	showRow( $lang['opt_sys_chars'], $lang['opt_sys_charsd'], "<input type=\"text\" class=\"form-control\" name=\"save_con[charset]\" value=\"{$config['charset']}\">" );
