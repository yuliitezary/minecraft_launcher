	  ...
	  <div class="col-md-5">
		<div class="form-group">
		  ...
		  <div class="col-md-10">
			...
			<div class="checkbox mt-5"><label><input class="icheck" type="checkbox" id="search_full_name" name="search_full_name" value="1" {$ifsfn}>{$lang['search_full_name']}</label></div>
		  </div>
		 </div>
		<div class="form-group">
...
HTML;
		...
		if( $config['charset'] != 'utf-8' ) {
			...
			if( function_exists( 'mb_convert_encoding' ) ) {
				...
				$lang['u_export_title_2'] = mb_convert_encoding( $lang['u_export_title_2'], "UTF-8", $config['charset'] );
				$lang['u_export_title_3'] = mb_convert_encoding( $lang['u_export_title_3'], "UTF-8", $config['charset'] );
				$lang['u_export_title_4'] = mb_convert_encoding( $lang['u_export_title_4'], "UTF-8", $config['charset'] );
				
			} elseif( function_exists( 'iconv' ) ) {
		
				$lang['u_export_title_2'] = iconv( $config['charset'], "UTF-8", $lang['u_export_title_2'] );
				$lang['u_export_title_3'] = iconv( $config['charset'], "UTF-8", $lang['u_export_title_3'] );
				$lang['u_export_title_4'] = iconv( $config['charset'], "UTF-8", $lang['u_export_title_4'] );
		...
		while( $row = $db->get_row() ) {
			...
			if( $config['charset'] != 'utf-8' ) {
				if( function_exists( 'mb_convert_encoding' ) ) {
					...
					$row['name'] = mb_convert_encoding( $row['name'], "UTF-8", $config['charset'] );
					$row['fullname'] = mb_convert_encoding( $row['fullname'], "UTF-8", $config['charset'] );
					$row['email'] = mb_convert_encoding( $row['email'], "UTF-8", $config['charset'] );
					
				} elseif( function_exists( 'iconv' ) ) {
		
					$row['name'] = iconv( $config['charset'], "UTF-8", $row['name'] );
					$row['fullname'] = iconv( $config['charset'], "UTF-8", $row['fullname'] );
					$row['email'] = iconv( $config['charset'], "UTF-8", $row['email'] );
...
function DelIgnorePM( id, text ){
    ...
    DLEconfirm( text, '{$lang['p_confirm']}', function () {
		...
		$.get("engine/ajax/pm.php", { id: id, action: "del_ignore", user_hash: '{$dle_login_hash}', skin: '{$config['skin']}' }, function(data){
